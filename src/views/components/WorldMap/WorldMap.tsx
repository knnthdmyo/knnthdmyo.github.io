...
const WorldMap = memo(
  ({ locations: customLocations, title }: WorldMapProps) => {
    const {
      locations,
      mapConfig,
      hoveredLocation,
      handleLocationHover,
      handleLocationLeave,
    } = useWorldMap(customLocations);

    return (
      <div className="relative w-full max-w-5xl mx-auto px-4">
        {title && (
          <p className="text-[10px] uppercase tracking-widest text-surface-500 dark:text-surface-400 mb-4 text-center">
            {title}
          </p>
        )}
          ...
          >
            <Geographies geography={mapConfig.geoUrl}>
              {({ geographies }) =>
                geographies
                  .filter(
                    (geo) =>
                      !mapConfig.excludeRegions.includes(geo.properties.name)
                  )
                  .map((geo) => (
                    <Geography
                      key={geo.rsmKey}
            ...
            {locations.map((loc, index) => (
              <Marker key={loc.name} coordinates={loc.coordinates}>
                ...
                <circle
                  r={7}
                  fill={hoveredLocation === loc.name ? '#525252' : '#262626'}
                  className="cursor-pointer transition-all duration-200"
                  onMouseEnter={() => handleLocationHover(loc.name)}
                  onMouseLeave={handleLocationLeave}
                  style={{
                    transform:
                      hoveredLocation === loc.name ? 'scale(1.3)' : 'scale(1)',
                  }}
                />
                {/* Tooltip */}
